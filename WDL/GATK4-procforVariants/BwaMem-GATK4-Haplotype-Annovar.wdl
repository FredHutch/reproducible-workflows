...
## - batch file indicating locations of:
##      - one unmapped bam for each sample (SM) 
##      - bed file associated with panel or exome
##
### Note: 
...
##
## Output :
## - vcf from GATK Haplotype caller
## - vcf from samtools mpileup
## - annotated variants from GATK Haplotype caller
## - annotated variants from samtools mpileup
## - QC Depth of Coverage report
##
...
## - TBD
## Runtime parameters are for FH AWS 
...
# Generate haplotype caller vcf
    call HaplotypeCaller {
      input:
        ...
        output_filename = base_file_name + ".Haplo.vcf.gz",
        ref_dict = ref_dict,
        ref_fasta = ref_fasta,
        ref_fasta_index = ref_fasta_index,
        dbSNP_vcf = dbSNP_vcf,
    ...
    call samtoolsMpileup {
      input:
        ...
        input_bam_index = ApplyBQSR.recalibrated_bai,
        interval_list = bedLocation,
        output_filename = base_file_name + ".Sam.vcf.gz",
        ref_dict = ref_dict,
        ref_fasta = ref_fasta,
        ref_fasta_index = ref_fasta_index,
        dbSNP_vcf = dbSNP_vcf,
    ...
    call samtoolsMpileup {
      input:
        input_vcf = ApplyBQSR.recalibrated_bam,
        interval_list = bedLocation,
        output_filename = base_file_name + ".Sam.vcf.gz",
        ref_dict = ref_dict,
        ref_fasta = ref_fasta,
    ...
    call annovar {
      input:
        ...
        output_GATK_filename = base_file_name + ".GATK.annotated.txt",
        output_Sam_filename = base_file_name + ".Sam.annotated.txt",
        docker = gotc_docker,
        gotc_path = gotc_path,
        disk_size = small_disk,
        preemptible_tries = preemptible_tries
  ...
  output {
    ...
    File GATK_annotated = annovar.output_GATK_annotated
    File Sam_annotated = annovar.output_Sam_annotated
...
task SamToFastqAndBwaMem {
  ...
  command <<<
    set -o pipefail
    set -e
### bwa mem -K 100000000 -p -v 3 -t 16 -Y $bash_ref_fasta" (bwa_commandline)
...
perl $ANNOVAR/table_annovar.pl QC/${1}.raw.snps.indels.vcf $ANNOVARDB \
 ...
 perl $ANNOVAR/table_annovar.pl QC/${1}.raw.snps.indels.vcf $ANNOVARDB \
  ...
  output {
    File output_GATK_annotated = "${output_GATK_filename}"
    File output_Sam_annotated = "${output_Sam_filename}"
  }
}